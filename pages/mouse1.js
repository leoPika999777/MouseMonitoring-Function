  ...
  return (
    <div className={styles["face"]}>
      {Array.from({ length: 3 }).map((_, index) => (
        <div key={index} className={styles.eye} ref={eyeRefs.current[index]} style={{ left: [150, 300, 450][index], top: [300, 200, 300][index], transform: eyeTransforms[index] }}>
          <div className={styles["eye-white"]}></div>
          <div className={styles["eye-black"]}></div>
        </div>
      ))}
      {showHello && <div  className={styles["hello"]} >你要離開了嗎</div>}
    </div>
  );
}
